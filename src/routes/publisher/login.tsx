...
export const Route = createFileRoute("/publisher/login")({
  ...
  component: PublisherLoginPage,
});

type PublisherRole = "publisher" | "author";
...
function PublisherLoginPage() {
  ...
  const [selectedRole, setSelectedRole] = useState<PublisherRole>("publisher");
  ...
  // Institution / Publisher Branding
  const publisherOrg = {
    name: "Vimala Publications & Digital Press",
  ...
  const handleOtpChange = (index: number, value: string) => {
    ...
    if (value && index < 3) {
      const nextInput = document.getElementById(`pub-otp-${index + 1}`);
      nextInput?.focus();
    }
  };
  ...
  const handleOtpKeyDown = (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Backspace" && !otpValues[index] && index > 0) {
      const prevInput = document.getElementById(`pub-otp-${index - 1}`);
      prevInput?.focus();
    }
  };
  ...
  const handleForgotOtpChange = (index: number, value: string) => {
    ...
    if (value && index < 3) {
      const nextInput = document.getElementById(`pub-forgot-otp-${index + 1}`);
      nextInput?.focus();
    }
  };
  ...
  return (
    <div className="min-h-screen w-full bg-background text-foreground flex flex-col items-center p-4 sm:p-6 md:p-8 select-none">
      ...
      <header className="w-full max-w-[540px] flex items-center justify-between mb-4 sm:mb-6 pt-1 sm:pt-2 shrink-0">
        <Link to="/" id="btn-pub-login-logo-link" className="flex items-center gap-2">
        ...
        <div className="bg-card border border-border rounded-3xl p-5 sm:p-7 pt-4 sm:pt-5 shadow-xl relative overflow-hidden space-y-5">
          ...
          {viewMode === "login" && (
            <div className="space-y-5 animate-in fade-in duration-200">
              {/* Vimala College Banner Logo */}
              <div className="flex flex-col items-center text-center space-y-2 -mt-2">
                <div className="bg-white dark:bg-white/95 rounded-xl px-4 py-2 shadow-xs border border-border/40 w-full max-w-[320px] flex items-center justify-center">
                  <img
                ...
                <div className="space-y-0.5 pt-1">
                  ...
                  <p className="text-sm font-semibold text-muted-foreground/90">
                    Content & Royalty Management Portal
                  </p>
                </div>
              </div>

              {/* 1. ROLE SELECTION (Publisher vs Author) */}
              ...
              <div className="space-y-4 pt-1">
                {/* PRIMARY METHOD: ID/EMAIL & PASSWORD */}
                    ...
                    <div className="space-y-1.5 text-left">
                      ...
                      <div className="text-right pt-0.5">
                        <button
                          ...
                          id="btn-open-pub-forgot-password"
              ...
              {!forgotSubmitted ? (
                <div className="space-y-6 text-center py-2">
                  ...
                  <button
                    ...
                    disabled={isSubmitting}
                    id="btn-pub-submit-forgot-password"
      ...
      <footer className="w-full text-center text-xs text-muted-foreground mt-auto pt-8 pb-4 flex items-center justify-center gap-2 shrink-0">
        ...
        <span>© 2026 {publisherOrg.name} · Powered by PixelBooks</span>
      </footer>
    </div>
  );
}
