...
function EditPromoCodePage() {
  ...
  const parseDateString = (isoStr?: string, formattedStr?: string) => {
    if (isoStr) {
  ...
  const initialFrom = parseDateString(existingPromo?.startDateISO, existingPromo?.start);
  const initialTo = parseDateString(existingPromo?.endDateISO, existingPromo?.end) || initialFrom;
  ...
  useEffect(() => {
    if (existingPromo) {
      ...
      const from = parseDateString(existingPromo.startDateISO, existingPromo.start);
      const to = parseDateString(existingPromo.endDateISO, existingPromo.end) || from;
  ...
  const handleSave = () => {
    ...
    const startDateISO = fromDate ? format(fromDate, "yyyy-MM-dd") : existingPromo.startDateISO;
    const endDateISO = toDate ? format(toDate, "yyyy-MM-dd") : existingPromo.endDateISO;
    const start = fromDate ? format(fromDate, "MMM dd, yyyy") : existingPromo.start;
    const end = toDate ? format(toDate, "MMM dd, yyyy") : existingPromo.end;
