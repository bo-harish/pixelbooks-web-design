...
export const Route = createFileRoute("/pb-admin-lib/terms-conditions_/$id")({
  component: EditTermsConditionsPage,
});
...
function EditTermsConditionsPage() {
  ...
  const [content, setContent] = useState(termsItem.content);
  ...
  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    toast.success(`Terms & Conditions for ${selectedRole} updated successfully!`);
    navigate({ to: "/pb-admin-lib/terms-conditions" });
  };

  return (
    <AppShell
      title="Edit Terms & Conditions"
      subtitle={`Edit terms & conditions details for ${selectedRole}.`}
    >
      <div className="p-4 sm:p-6 md:p-8 max-w-5xl space-y-6">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          >
            {status === "Active" ? (
              <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-500/20 bg-emerald-500/10 px-3.5 py-1.5 text-xs font-semibold text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/20 transition-colors">
                <CheckCircle2 size={13} />
                Active
              </span>
            ) : (
              <span className="inline-flex items-center gap-1.5 rounded-full border border-amber-500/20 bg-amber-500/10 px-3.5 py-1.5 text-xs font-semibold text-amber-600 dark:text-amber-400 hover:bg-amber-500/20 transition-colors">
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            <button
              type="button"
              onClick={() => navigate({ to: "/pb-admin-lib/terms-conditions" })}
