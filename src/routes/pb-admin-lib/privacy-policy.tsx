...
export const Route = createFileRoute("/pb-admin-lib/privacy-policy")({
  component: PrivacyPolicyListPage,
});

function PrivacyPolicyListPage() {
  const navigate = useNavigate();
  const [policies] = useState<PrivacyPolicyItem[]>(initialPrivacyPolicies);
  ...
  }, [policies, selectedRole, selectedStatus]);

  return (
    <AppShell
      title="Privacy Policy"
      subtitle="Manage system-wide privacy policy terms across Publisher, Customer, Author, Library, and Library User portals."
    >
      <div className="p-4 sm:p-6 md:p-8 space-y-6">
        ...
        <div className="flex flex-col gap-3 rounded-xl border border-border bg-card p-4 md:flex-row md:flex-nowrap md:items-center md:justify-between shadow-2xs">
          ...
          {/* Create New Button */}
          <Link
            to="/pb-admin-lib/privacy-policy/new"
                ...
                {filteredPolicies.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="py-12 text-center text-sm text-muted-foreground">
                      No privacy policies found matching your search criteria.
                    </td>
                  </tr>
                ) : (
                  filteredPolicies.map((item) => (
                    <tr
                      ...
                      onClick={() =>
                        navigate({
                          to: "/pb-admin-lib/privacy-policy/$id",
                          params: { id: item.id },
                        })
                      }
